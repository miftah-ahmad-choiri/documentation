...
- **Third-person singular verbs** require `-s/-es`.  
...
- **Always plural:** *both, few, many, several*  
  ✅ *Both **are** excited.*  
- **Depends on the noun after "of":* *some, all, none, most, half*  
...
- **Possessive forms** use `'s` for singular and `s'` for plural.  
- **Count nouns** can be singular/plural; **noncount nouns** do not have a plural form.  
...
- **Nouns ending in s, sh, ch, x, o:** Add `-es` → *box → boxes, potato → potatoes*  
...
🔹 **Rules:**  
- **Singular nouns** → Add `'s` (*The cat’s food, John’s book*)  
- **Plural nouns ending in -s** → Add **only** `'` (*The dogs’ owner, the teachers’ lounge*)  
...
- **"A" is for consonant sounds** (`a book`), **"An" is for vowel sounds** (`an apple`).  
- Use **"A" or "An" for first mentions**, then **"The"** for repeated mentions.  
...
- `A` is used before **words that begin with a consonant sound** (*a book, a cat, a university*).  
- `An` is used before **words that begin with a vowel sound** (*an apple, an hour, an elephant*).  
...
| `-ed` Adjectives (feelings) | `-ing` Adjectives (causing feelings) |
|----------------------------|---------------------------------|
...
💡 **Rule of thumb:**  
- `-ed` → How **someone** feels (*I am confused.*)  
- `-ing` → How **something** makes you feel (*The lesson is confusing.*)  
...
🔹 After **"need"**, we use:  
1. **Passive infinitive ("to be + V3")**  
2. **Gerund in passive form ("being + V3")**  
...
1. **Gerund (-ing) → Ongoing action**  
2. **Base verb (bare infinitive) → Completed action**  
...
1. A **comma + coordinating conjunction** (*and, but, or, so, yet, for, nor*).  
2. A **semicolon (;)** without a conjunction.  
...
🔹 There are **three** main categories of connectives:  
  1. **Cause and Effect** (e.g., *because, due to, therefore, consequently*)  
  2. **Contrast** (e.g., *however, although, but, despite*)  
  3. **Condition** (e.g., *if, unless, otherwise, or else*)  
